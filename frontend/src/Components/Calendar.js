      ...
      <div className="CalendarContainer">
        ...
        <div className={`CalendarHeader ${classNameHeader}`}>
          <div className="CalendarMonth">
            <button
              className={`CalendarMonthBtn ${
                selectedMonth <= 0 ? "CalendarBtnsNoDisplay" : ""
              }`}
              onMouseOver={() => handleMouseOver("previous")}
            ...
            <button
              className={`CalendarMonthBtn ${
                selectedMonth >= 11 ? "CalendarBtnsNoDisplay" : ""
              }`}
          ...
          <div className="CalendarYear">
            ...
            <button
              className={`CalendarYearBtn ${
                selectedYear >= 2100 ? "CalendarBtnsNoDisplay" : ""
              }`}
            ...
            {dataArray.map((_, idx) => {
              if (idx === 0) {
                ...
                return (
                  <CalendarRow
                    key={idx}
                    day_array={dataArray.slice(idx, idx + 7)}
                    selectedMonth={selectedMonth}
                    selectedYear={selectedYear}
                    handleClick={handleClick}
                  ></CalendarRow>
...
const CalendarRow = ({
  ...
  const return_if_current_day = (day_nr) => {
    const date = new Date();
    if (date.getDate() !== day_nr) return false;
  ...
  return (
    <div className="CalendarRow">
      {day_array.map((day, id) => {
        ...
        return (
          <CalendarDay
            dayNumber={day.day_nr}
            key={id}
            isInvalid={
              is_from_other_month || return_if_before_start_day(day.day_nr)
            }
            isStartDay={isStartDay}
            isDDay={isDDay}
            isNormalTrainingDay={
              [0, 1, 2, 3, 4].includes(id) &&
              !(isStartDay || isDDay) &&
              return_if_no_special_day(day.day_nr) &&
              !is_from_other_month
            }
            ...
            isDefumoxanStartDay={
              day.day_nr === 25 && selectedMonth === 2 && selectedYear === 2023
            }
            isDefumoxanEndCigarettesDay={
              day.day_nr === 30 && selectedMonth === 2 && selectedYear === 2023
            }
            isDefumoxanEndDay={
              day.day_nr === 18 && selectedMonth === 3 && selectedYear === 2023
            }
            isCurrentDay={return_if_current_day(day.day_nr)}
            handleClick={handleClick}
          ></CalendarDay>
        );
      })}
...
const CalendarDay = ({
  ...
  return (
    <div
      className={`CalendarDay ${isInvalid ? "CalendarDayInvalid" : ""} ${
        isStartDay ? "CalendarStartDay" : ""
      } ${isDDay ? "CalendarDDay" : ""}
        ${isNormalTrainingDay ? "CalendarNormalTrainingDay" : ""} ${
        isSaturdayTrainingDay ? "CalendarSaturdayTrainingDay" : ""
      } 
